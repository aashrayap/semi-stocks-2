...
| `proposal` | 4 | Thesis proposals, shown as evidence context but not graph edges |
...
The current app is useful for inspecting the contract, but not polished.

Known rough areas:

- graph is dense because co-position edges create many pairs
- graph layout is deterministic but simple
- table columns are functional, not final
- profile panel is useful but still shallow
- source-document view is basic
- no browser screenshot QA has been done
...
1. Reduce graph clutter with edge thresholds or support-family toggles.
2. Improve company and edge profile sections.
3. Make table columns more specific per tab.
4. Add a clear legend explaining contextual evidence edges.
5. Run browser/screenshot QA on the web reader.
